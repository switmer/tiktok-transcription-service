...
def process_fantasy_football_video(url):
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            future_to_chunk = {executor.submit(transcribe_audio_cached, chunk): chunk for chunk in chunks}
            ...
            with open(transcript_path, "w", encoding="utf-8") as f:
                ...
                for future in concurrent.futures.as_completed(future_to_chunk):
                    ...
                    try:
                        chunk_transcript, chunk_duration = future.result()
                        if chunk_transcript and 'segments' in chunk_transcript:
                            ...
                            logging.warning(f"Unexpected transcript format for chunk {chunk}")
...
def print_elapsed_time(start_time):
    ...
    print(f"Total processing time: {int(hours):02d}:{int(minutes):02d}:{seconds:05.2f}")
