-- Fix update_transcription function overload issue
-- This migration ensures only ONE version of the function exists

-- Drop ALL possible versions of the function (text and uuid variants)
DROP FUNCTION IF EXISTS update_transcription(text, jsonb);
DROP FUNCTION IF EXISTS update_transcription(uuid, jsonb);
DROP FUNCTION IF EXISTS public.update_transcription(text, jsonb);
DROP FUNCTION IF EXISTS public.update_transcription(uuid, jsonb);

-- Recreate the function with ONLY UUID parameter
...
Accepts UUID task_id (not TEXT) to avoid function overloading ambiguity.
Used by Edge Functions to handle problematic updates.';

-- Success message
DO $$
BEGIN
    RAISE NOTICE '✅ update_transcription function fixed!';
    RAISE NOTICE 'All overloaded versions dropped, only UUID version exists now.';
    RAISE NOTICE 'PostgREST function resolution error should be resolved.';
END $$;
