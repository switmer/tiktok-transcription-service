...
class RapidAPISpotifyV2Adapter(TikTokAPIAdapter):
    """
    Fallback Spotify adapter using real-time-spotify-data-scraper.p.rapidapi.com.
    Returns full metadata + audio.items[] with scdn.co URLs.
    Note: p.scdn.co/mp3-preview URLs may 404 — this adapter works best as metadata fallback.
    """

    def __init__(self, api_key: str, host: str = "real-time-spotify-data-scraper.p.rapidapi.com"):
        super().__init__(
            name=f"RapidAPI-SpotifyV2-{host}",
            api_key=api_key,
    ...
    def get_video_info(self, video_url: str) -> APIResponse:
        try:
            ...
            logger.info(f"Fetching Spotify episode via real-time scraper: {episode_id}")
            resp = requests.get(
    ...
    def _transform_response(self, episode: Dict, original_url: str, episode_id: str) -> Dict[str, Any]:
        try:
            # Extract audio URL — prefer MP4_128 format
            ...
            logger.warning(f"Error transforming SpotifyV2 response: {e}")
            return {
                'id': episode_id,
